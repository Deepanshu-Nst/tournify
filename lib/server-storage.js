...
export async function getTournamentsByOrganizer(organizerId) {
  const tournaments = await getAllTournaments()
  // Only return tournaments that are explicitly owned by this organizer
  return tournaments.filter((tournament) => Boolean(tournament.organizerId) && tournament.organizerId === organizerId)
}
...
export async function initializeStorage() {
  ...
  const sampleTournaments = [
    ...
    },
    {
      ...
      prizePool: "$2,500",
      registrationType: "Free",
      image: "https://us-west-2-epicgames.graphassets.com/A0Gpf4ZpfQzaTePr3NRc1z/resize=fit:clip,height:720,width:1280/output=format:webp/QXjLW0nQC2NxxI8osiJk",
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
    },
    {
      ...
      prizePool: "$5,000",
      registrationType: "Free",
      image: "https://play-lh.googleusercontent.com/OtFyhtgvo8pv3oB4ZR-nYDxHMfwoYgjbZH-iBdoDm60-d67tOqJrLCnTzL9Be3VYbgpTZtTMx8o=w600-h300-pc0xffffff-pd",
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
    },
    {
      ...
      prizePool: "$2,000",
      registrationType: "Free",
      image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ0lQ0wdUUCkN1vaOPZama_YwXrns7p1miljg&s",
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
    },
    {
  ...
  if (tournaments.length < sampleTournaments.length) {
    ...
    // Update existing sample tournaments if newer
    let changed = false
    ...
    if (changed) await setData(STORAGE_KEYS.TOURNAMENTS, tournaments)
  }
}
